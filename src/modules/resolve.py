...
# Enumerate Host
def request_target(target, timeout, headers, verbose, include_filter, exclude_filter):
...
def filter_out(target, req, exclude, include):
    ...
    if include is not None:
        if include == 'INFO' and req.status_code in range(100, 199) :
            print(f'{cli.blue}INFO{cli.endc} - {target} [{cli.blue}{req.status_code}{cli.endc}] {srv}')
        elif include == 'SUCCESS' and req.status_code in range(200, 299):
            print(f'{cli.green}SUCCESS{cli.endc} - {target} [{cli.green}{req.status_code}{cli.endc}] {srv}')
        elif include == 'REDIRECTION' and req.status_code in range(300, 399):
            print(f'{cli.yellow}REDIRECTION{cli.endc} - {target} [{cli.yellow}{req.status_code}{cli.endc}] {srv} {cli.yellow}→{cli.endc} {req.headers["location"]}')
        elif include == 'CLIENT_ERROR' and req.status_code in range(400, 499):
            print(f'{cli.purple}CLIENT_ERROR{cli.endc} - {target} [{cli.purple}{req.status_code}{cli.endc}] {srv}')
        elif include == 'SERVER_ERROR' and req.status_code in range(500, 599):
            print(f'{cli.red}SERVER_ERROR{cli.endc} - {target} [{cli.red}{req.status_code}{cli.endc}] {srv}')
        else:
            pass
    # Exclude
    if include is None:
        ...
        if type(target) is list:
            if args.common_ports:
                ...
                with ThreadPoolExecutor(max_workers=args.threads) as executor:
                    for url in target:
                        for port in ports:
                            threads.append(executor.submit(request_target, target=f'http://{url}:{port}', timeout=args.timeout, headers={'User-Agent': args.user_agent}, verbose=args.verbose, include_filter=args.include_filter, exclude_filter=args.exclude_filter))
                            threads.append(executor.submit(request_target, target=f'https://{url}:{port}', timeout=args.timeout, headers={'User-Agent': args.user_agent}, verbose=args.verbose, include_filter=args.include_filter, exclude_filter=args.exclude_filter))
            else:
                threads = []
                with ThreadPoolExecutor(max_workers=args.threads) as executor:
                    for url in target:
                        threads.append(executor.submit(request_target, target=f'http://{url}', timeout=args.timeout, headers={'User-Agent': args.user_agent}, verbose=args.verbose, include_filter=args.include_filter, exclude_filter=args.exclude_filter))
                        threads.append(executor.submit(request_target, target=f'https://{url}', timeout=args.timeout, headers={'User-Agent': args.user_agent}, verbose=args.verbose, include_filter=args.include_filter, exclude_filter=args.exclude_filter))
        if type(target) is str:
            if args.common_ports:
                threads = []
                with ThreadPoolExecutor(max_workers=args.threads) as executor:
                    for port in ports:
                        threads.append(executor.submit(request_target, target=f'http://{target}:{port}', timeout=args.timeout, headers={'User-Agent': args.user_agent}, verbose=args.verbose, include_filter=args.include_filter, exclude_filter=args.exclude_filter))
                        threads.append(executor.submit(request_target, target=f'https://{target}:{port}', timeout=args.timeout, headers={'User-Agent': args.user_agent}, verbose=args.verbose, include_filter=args.include_filter, exclude_filter=args.exclude_filter))
            else:
                threads = []
                with ThreadPoolExecutor(max_workers=args.threads) as executor:
                    threads.append(executor.submit(request_target, target=f'http://{target}', timeout=args.timeout, headers={'User-Agent': args.user_agent}, verbose=args.verbose, include_filter=args.include_filter, exclude_filter=args.exclude_filter))
                    threads.append(executor.submit(request_target, target=f'https://{target}', timeout=args.timeout, headers={'User-Agent': args.user_agent}, verbose=args.verbose, include_filter=args.include_filter, exclude_filter=args.exclude_filter))
